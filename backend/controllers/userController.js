...
const generateToken=(_id, email)=>{
    return jwt.sign({_id, email}, env.JWT_SECRET, {expiresIn:'2d'})
}
    ...
    try {
        ...
        .then(user=>{
            ...
            .then(passwordEqual=>{
                ...
                return res.status(200).send({ msg:"signin success", email, token});
            })
            .catch(error=>{
    ...
    try {
        ...
        if(userId){
            AppUser.updateOne({_id:userId}, userChanges, function(error, user){
                ...
                return res.status(201).send({ msg : "user updated success"});
            })
        }
        else{
...
const resetPassword=async(req, res)=>{
    if(req.app.locals.resetSession){
        ...
        return res.status(201).send({msg:"access allowed"})
    }
    return res.status(440).json({error: "session expired"});
}

module.exports={signup, generateOTP,signupMail, verifyOTP, auth, resetSession , signin, getUser, updateUser, resetPassword, verifyUser}
